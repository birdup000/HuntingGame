...
class Player:
    ...
    def __init__(self, app, terrain_height: float = 0.0, setup_controls: bool = True, weapon: Optional[Weapon] = None):
        """Initialize the player with the Panda3D application."""
        self.app = app
        
        # Start above terrain height to avoid clipping
        # Use provided terrain_height parameter or calculate from terrain
        if hasattr(self, 'get_terrain_height'):
            start_height = self.get_terrain_height(0, 0)
        else:
            start_height = terrain_height
        self.position = Point3(0, 0, start_height + 1.8)  # 1.8 units above terrain (more natural height)
        ...
        # Store initial camera state for better reset behavior
        self.initial_camera_pos = None
        self.initial_camera_hpr = None
        self.camera_target_height = terrain_height + 2.0  # Cache for consistent resets

        # Camera setup with improved positioning and state preservation
        self.camera_node = getattr(self.app, 'camera', None)
        if self.camera_node is not None:
            # Save initial camera state for resets
            self.initial_camera_pos = self.camera_node.getPos()
            self.initial_camera_hpr = self.camera_node.getHpr()
        ...
        try:
            ...
            print(f"Error creating collision_manager: {e}")
            self.collision_manager = None

        # Collision setup (always needed for movement)
        if hasattr(self.app, 'render') and self.model is not None:
        ...
        elif not hasattr(self.app, 'render'):
            print("Warning: Render not available - no collision setup")
        elif self.model is None:
            print("Warning: Model not available - no collision setup")

        # Add camera reset method for better recovery from window focus issues
    def _update_camera_position(self):
        """Update camera position relative to player."""
        if self.camera_node:
            # Position camera properly above terrain
            camera_height = self.position.z + 2.0  # At eye level
            self.camera_node.setPos(self.position.x, self.position.y - self.camera_distance, camera_height)
    ...
    def reset_camera(self):
        ...
        if self.initial_camera_pos and self.initial_camera_hpr and self.camera_node:
            self.camera_node.setPos(self.initial_camera_pos)
            self.camera_node.setHpr(self.initial_camera_hpr)
        ...
        self.setup_controls = False  # Initialize the flag
        print("WEAPON CREATED:", self.weapon)

        # Input setup (only for controls)
        if setup_controls:
            self.setup_controls = True
            self._setup_input_controls()
        else:
            # Set up basic movement state for non-controlled mode
            self.movement = {
                'forward': False,
                'backward': False,
                'left': False,
        ...
        print("Player constructor COMPLETED")
        print("Weapon:", hasattr(self, 'weapon'))
        print("Setup controls called:", setup_controls)
        if hasattr(self, 'weapon'):
            print("Weapon value:", self.weapon)

    def _setup_input_controls(self):
        """Internal method to set up keyboard and mouse controls."""
    ...
    def setup_controls(self):
        """Set up keyboard and mouse controls (public method)."""
        self._setup_input_controls()
        if not hasattr(self.app, 'accept') or not hasattr(self.app, 'taskMgr'):
            print("Warning: Input system not available - running in headless mode")
            self.movement = {
    ...
    def update(self, dt):
        ...
        if (self.collision_traverser is not None and 
            self.collision_handler is not None and
            hasattr(self.app, 'render')):
